...
abstract class GridView extends View
{
    ...
    protected function addDefaultActions($routeName, $add = true, $show = true, $edit = true, $delete = true, $changeStatus = false)
    {
        ...
        if ($show) {
            $this->addAction('show', 'Visualizar', 'sys_'.$routeName.'_show', 'GET', false, 'eye', "var(--blue)");
        }

        if ($edit) {
            $this->addAction('edit', 'Editar', 'sys_'.$routeName.'_edit', 'GET', false, 'pencil', "var(--orange)");
        }

        if ($delete) {
            $this->addAction('delete', 'Excluir', 'sys_'.$routeName.'_delete', 'DELETE', false, 'trash', "var(--red");
        }
    ...
    protected function addMetric($label, $value, $format = null)
    {
        $this->getViewComponent()->addMetric($label, $value, $format);
    }
